...
# Query ASP resources that have no sites and no cleanup tag assigned
$AzureResources = Search-AzGraph -Query "where type == 'microsoft.web/serverfarms' and properties.numberOfSites=='0' and tags.cleanup==''" -First 1000
...
# Query ASP resources that have sites and a cleanup tag with unattached in value
$AzureResources = Search-AzGraph -Query "where type == 'microsoft.web/serverfarms' and properties.numberOfSites!='0' and tags.cleanup=~'unattached'" -First 1000
